...
Yet you don't see any dependencies on Tez, Hadoop, HDFS etc. That is because the environment is loaded using standard Java mechanisms for loading services and providers allowing bootstrapping of different execution providers without requiring any changes to the end user code. API also provides for custom _FileSystemProviders_ allowing various file systems to be represented as 
_java.nio.file.FileSystem_, thus keeping your code clean and concise.
...
Which allows variaty of providers to be implemented under the same API giving end user out most fliexibility on choosing provider