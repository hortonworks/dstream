...
//		
//		Future<Stream<Stream<Entry<String, List<Tuple2<String, String>>>>>> resultFuture = one
////		Future<Stream<Stream<Tuple2<String, String>>>> resultFuture = one
//				.join(two).on(t2 -> t2._1().split("\\s+")[0].equals(t2._2().split("\\s+")[2]))
//				.aggregateValues(t2 -> t2._1().split("\\s+")[0], t2 -> t2)
//			.executeAs("Join");
//		
//		Stream<Stream<Entry<String, List<Tuple2<String, String>>>>> resultPartitionsStream = resultFuture.get();
